...
def test_user_email_validation():
    ...
    assert user.is_valid_email("test@example.com") is True, "Valid email should pass"
    assert user.is_valid_email("invalid.email") is False, "Invalid email should fail"
    assert user.is_valid_email("no@domain") is False, "Email without TLD should fail"
    assert user.is_valid_email("@example.com") is False, "Email without local part should fail"
...
def test_user_validation_errors():
    ...
    try:
        # Empty first name
        user = UserModel(first_name="", last_name="Doe", email="test@example.com")
    ...
    try:
        # Invalid email
        user = UserModel(first_name="John", last_name="Doe", email="invalid-email")
...
def test_user_name_length_validation():
    ...
    try:
        ...
        long_name = "A" * 51
        user = UserModel(first_name=long_name, last_name="Doe", email="test@example.com")
...
def test_place_basic_creation():
    ...
    place = PlaceModel(title="Cozy Apartment", description="A nice place", 
...
def test_multiple_places_same_owner():
    ...
    places = [
        PlaceModel(title="Apartment 1", price=100, latitude=0, longitude=0, owner=owner),
        PlaceModel(title="Apartment 2", price=150, latitude=1, longitude=1, owner=owner),
        PlaceModel(title="Villa", price=500, latitude=2, longitude=2, owner=owner)
    ]
    ...
    assert all(p.owner.email == "multi@example.com" for p in places), "All should link to same owner"
...
def test_shared_amenities():
    ...
    place1 = PlaceModel(title="Place 1", price=100, latitude=0, longitude=0, owner=owner)
    place2 = PlaceModel(title="Place 2", price=200, latitude=1, longitude=1, owner=owner)
